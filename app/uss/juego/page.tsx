...
function JuegoContent() {
  ...
  useEffect(() => {
    ...
    try {
      const raw = window.sessionStorage.getItem(`uss-share:${participantId}`);
      if (!raw) return;
  ...
  const handleShare = async () => {
    ...
    try {
      const level = getTutorLevel(score);
      const speciesLabel = speciesCatalog[participant.species]?.label || "Tema";
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: "Mi resultado en USS",
          text: `${participant.name} obtuvo ${score}/${questions.length} en USS Fauna Silvestre`,
          files: [file],
        });
      } else {
...
async function generateStoryImage({
  ...
  return new Promise<File>((resolve, reject) => {
    canvas.toBlob((blob) => {
      ...
      resolve(new File([blob], "resultado-uss-story.png", { type: "image/png" }));
    }, "image/png");
  });
}
